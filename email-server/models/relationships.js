...
const checkRelationship = async(node1,node2,typeOfRelationship)=>{
  let success = false;
  const session= connection().run(`
  Match (n:${node1._labels[0]})-[r:${typeOfRelationship}]->(m:${node2._labels[0]})
    where r is null return n,m
  `).then(result => {
...
async function create(node1,node2,typeOfRelationship){
    checkRelationship(node1,node2,typeOfRelationship);
    let success = false;
    const session = connection()
    .run(`Match (n:${node1._labels[0]}),(m:${node2._labels[0]})
     where id(n)=${node1.id} and id(m)=${node2.id}  CREATE(n)-[r:${typeOfRelationship}]->(m)
